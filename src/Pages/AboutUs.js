...
function AboutUs() {
  ...
  const newsButtonsRef = useRef(null);
  const isDragging = useRef(false);
  const startX = useRef(0);
  const scrollLeft = useRef(0);

  // ✅ Enable horizontal scroll with mouse left button (drag)
  const handleMouseDown = (e) => {
    isDragging.current = true;
    startX.current = e.pageX - newsButtonsRef.current.offsetLeft;
    scrollLeft.current = newsButtonsRef.current.scrollLeft;
  };
  ...
  const handleMouseMove = (e) => {
    ...
    e.preventDefault();
    const x = e.pageX - newsButtonsRef.current.offsetLeft;
    const walk = (x - startX.current) * 1; // speed
    newsButtonsRef.current.scrollLeft = scrollLeft.current - walk;
  };
  ...
  return (
    <div className="about-page">
      ...
      <div className="news-section">
        ...
        {/* ✅ Horizontal Scrollable Buttons with Mouse Drag */}
        <div
          className="news-buttons"
          ref={newsButtonsRef}
